...
const handleFavoritesViewProduct = (productId) => {
  navigateTo(`/produk/${productId}`);
};

const handleFavoritesAddToCart = (productId) => {
  useSwal().showInfo("Fitur cart sedang dikembangkan");
};
...
<template>
  <div class="relative min-h-dvh text-slate-100 overflow-hidden">
    ...
    <section class="relative mx-auto max-w-7xl px-4 pt-12 sm:px-6 lg:px-8">
      <div class="py-8">
        <div
          class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300 mb-4"
        >
          <span class="h-2 w-2 rounded-full bg-sky-400"></span> Jelajahi Produk
          & Jasa
        </div>
        <h1
          class="text-balance text-4xl font-extrabold tracking-tight sm:text-5xl mb-4"
        >
          Temukan
          <span
        ...
        <p class="max-w-2xl text-pretty text-slate-300 mb-6">
          Ribuan produk dan jasa berkualitas dari para penjual terpercaya. Mulai
          dari makanan, merchandise, layanan desain, hingga event support.
        </p>
    ...
    <!-- Favorites Section - Reusable Component -->
    <ProductFavorites
      :perPage="5"
      @view-product="handleFavoritesViewProduct"
      @add-to-cart="handleFavoritesAddToCart"
    />
    ...
    <section class="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div class="grid gap-8 lg:grid-cols-4">
        ...
        <div class="lg:col-span-1">
          <div
            class="sticky top-24 rounded-2xl border border-white/10 bg-slate-900/60 p-6 backdrop-blur-xl"
          >
        ...
        <div id="products-section" class="lg:col-span-3 space-y-8">
          ...
          >
            ...
            >
              <div
                class="mb-4 aspect-square w-full rounded-xl bg-slate-800/50 animate-pulse"
              ></div>
              <div
                class="h-3 w-2/3 rounded bg-slate-700/50 mb-2 animate-pulse"
              ></div>
              <div
                class="h-3 w-1/3 rounded bg-slate-700/50 mb-3 animate-pulse"
              ></div>
              <div
                class="h-3 w-full rounded bg-slate-700/50 animate-pulse"
              ></div>
            </div>
          </div>
          ...
          >
            ...
            <p class="text-slate-400">
              Belum ada produk yang sesuai dengan pencarian atau filter Anda.
            </p>
          </div>
