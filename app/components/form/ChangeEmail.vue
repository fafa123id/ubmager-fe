...
const saveOtp = async () => {
  try {
    ...
    email.value = "";
    useSwal().close();
    useSwal().showSuccess("Email berhasil diubah.");
    useAuth().fetchUser();
  } catch (e) {
    useSwal().close();
    useSwal().showError(
      e.response?.data?.message || "Gagal mengubah email."
    );
  } finally {
    loading.value = false;
  }
...
const closeAndReset = async () => {
  const result = await useSwal().confirmAction(
    "Apakah Anda yakin ingin membatalkan perubahan email?"
  );
...
<template>
  <MyModal id="change-email-modal" :show="props.show" @close="closeAndReset">
    <div class="p-6">
      <form @submit.prevent="saveOtp">
        <h2 class="text-lg font-medium text-white">
          Masukkan OTP yang dikirim ke email kamu dan Email Baru!
        </h2>
        ...
        <div class="mt-6">
          <label class="mb-1 block text-xs text-slate-300">Email Baru</label>
          <input
            v-model="email"
            type="email"
            class="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-400 outline-none ring-1 ring-transparent focus:ring-sky-400 disabled:opacity-60"
